...
def draw_bounding_box_on_image(image,ymin,xmin,ymax,xmax,num,color='red',thickness=4,
                               use_normalized_coordinates=False):
    ...
    temp=int(top-(bottom-top))
    if temp>0 and temp<im_height:
        tfont=ImageFont.truetype('./font/Arial.ttf',int((bottom-top))-1)
        draw.text((left,temp),str(num),fill=(255,0,0),font=tfont)
